...
- **設定ダイアログ**: ツールバーの歯車から開く。`Appearance` で配色 (既定は OS の設定に追従。切り替え後も OS 側の変更を拾う) とエディタ / Markdown プレビューの文字サイズ、`About` でバージョンと依存パッケージのライセンス一覧を表示する
